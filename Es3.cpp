...
void setup() {
  ...
  while(!display.begin(SSD1306_SWITCHCAPVCC,oled)){
    Serial.println("Display initialization falied");
  }
  ...
  while(!bmp.begin()){                              // Controllo che il bmp sia collegato correttamente 
    Serial.print("Errore nel bmp");                 // Se viene rilevato un problema con il bmp stampo un messaggio di errore
  }
  SerialBT.connect("Piva_Schiavo");                 // Mi connetto al'Esp32 di Piva 
...
void loop() {
  if(SerialBT.connected()){                         // Controllo che sia connesso
      ...
      SerialBT.printf("Temperatura: %.2f \n",temp); // Trasmetto i dati della temperatura al bluetooth
      SerialBT.printf("Pressione: %.2f ",press);    // Trasmetto i dati della pressione al bluetooth
    ...
    display.clearDisplay();
    display.setCursor(0,0);
    display.print("Disconnesso");
    Serial.println("Disconnesso");
    display.display();
    SerialBT.connect("Piva_Schiavo");
    delay(200);
  }
  }
